...
// ================================================================================================
     ...
     * добавление элементов в конец массива не требует каких-то дополнительных
     ...
     * Остается проблема поиска нужной ноды, которая, как указано ранее, O(n), что
     * суммарно дает сложность O(1) + O(n) => O(n) для всех случаев, кроме вставки в
     * начало – всегда имеется ввиду ссылка на первый элемент и искать его не нужно. Для
    ...
    public void add (int value) { // Функция вставки в конец списка
        ...
        } else { // Добавляем наш элемент в конец списка, добавляя связь уже предыдущего tail с
            // нашим текущий
    ...
    public void delete(Node node) { // Функция удаления элемента из списка
        ...
        if (previous == null) {
            ...
            if (next == null) {
                ...
                tail = previous; // Кладём в tail значение предыдущее перед текущим tail
            } else {
...
// ================================================================================================
     ...
     * Берем первую ноду в списке и начинаем по очереди перебирать значения, пока
     * не найдем требуемый узел, либо список не закончится. Как не сложно догадаться,
     * подобный поиск будет иметь сложность O(n). Это значит, что все плюсы бинарного
    ...
    public Node find(int value) {
        ...
        while (currentNode != null) { // Цикл пока текущая Node не равна null, т.е. пока в
...
// ================================================================================================
     ...
     * списке.
     * А значит алгоритм будет представлять обычный перебор, где мы меняем
     * ссылки местами.
...
// ================================================================================================
    ...
    public Integer peek() {
        ...
        if (tail != null) {
            result = tail.value;
            tail.previous.next = null; // для предыдущего элемента он станет последним
            tail = tail.previous; // и сам tail меняем на предыдущий элемент
        }
        return result;
    }
}
